...
// Component to display known variants from ClinVar for a given gene
export default function KnownVariants({
  refreshVariants,// Function to refresh the list of variants
  showComparison,// Function to show comparison modal for a variant
  updateClinvarVariant,// Function to update a specific ClinVar variant
  clinvarVariants,// List of ClinVar variants
  isLoadingClinvar,// Loading state for ClinVar variants
  clinvarError,// Error state for ClinVar variants
  genomeId,// Genome identifier
  gene,// Gene information
}: {
  refreshVariants: () => void;// Function to refresh variants
  showComparison: (variant: ClinvarVariant) => void;// Function to show comparison modal
  updateClinvarVariant: (id: string, newVariant: ClinvarVariant) => void;// Function to update a ClinVar variant
  clinvarVariants: ClinvarVariant[];// List of ClinVar variants
  isLoadingClinvar: boolean;// Loading state
  clinvarError: string | null;// Error state
  genomeId: string;// Genome identifier
  gene: GeneFromSearch;// Gene information from search
}) {
  // Function to analyze a variant using Evo2  
  const analyzeVariant = async (variant: ClinvarVariant) => {
    let variantDetails = null;// Object to hold parsed variant details
    ...
    if (
      ...
      return;
    }// Return early if details are missing
    
    // Update variant state to indicate analysis is in progress
    ...
    try {
      const data = await analyzeVariantWithAPI({
        position: variantDetails.position,// Position of the variant
        alternative: variantDetails.alternative,// Alternative allele
        genomeId: genomeId,// Genome identifier
        chromosome: gene.chrom,// Chromosome information
      });

      // Update variant state with analysis results
      const updatedVariant: ClinvarVariant = {
        ...variant,
        isAnalyzing: false,// Analysis complete
        evo2Result: data,// Store Evo2 results
      };

      // Update the variant in the parent component
      updateClinvarVariant(variant.clinvar_id, updatedVariant);
      // Show comparison modal with updated variant
      showComparison(updatedVariant);
    } catch (error) {
      updateClinvarVariant(variant.clinvar_id, {
        ...variant,// Original variant data
        isAnalyzing: false,// Analysis complete
